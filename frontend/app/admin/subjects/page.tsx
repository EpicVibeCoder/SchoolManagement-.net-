...
import { ApiError, ClassDto, SubjectDto, del, get, post, put } from "@/lib/api";
...
export default function AdminSubjectsPage() {
  ...
  const loadSubjects = async () => {
    ...
    try {
      ...
      const data = await get<SubjectDto[]>(`/api/subjects${query}`);
      setSubjects(data);
    } catch (err) {
  ...
  const startEdit = (subject: SubjectDto) => {
    ...
    setFormError(null);
    reset({ name: subject.name, code: subject.code, classId: subject.classId });
  };
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="sm-card p-5 lg:order-2 h-fit">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-3" noValidate>
            ...
            <div>
              <label className="sm-label" htmlFor="name">Name</label>
            ...
            <div>
              <label className="sm-label" htmlFor="code">Code</label>
