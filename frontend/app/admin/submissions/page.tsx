      ...
      useEffect(() => {
            ...
            (async () => {
                  try {
                        ...
                        const results = await Promise.all(assignments.map((a) => get<SubmissionDto[]>(`/api/assignments/${a.id}/submissions`).catch(() => [] as SubmissionDto[])));
                        if (!cancelled) {
                              const flattened = results.flat().sort((a, b) => new Date(b.submittedAt).getTime() - new Date(a.submittedAt).getTime());
