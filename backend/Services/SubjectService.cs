...
{
    Task<List<SubjectDto>> ListAsync(Guid? classId, CancellationToken ct);
...
public class SubjectService : ISubjectService
{
    ...
    public async Task<List<SubjectDto>> ListAsync(Guid? classId, CancellationToken ct)
    {
        var query = _db.Subjects.Include(s => s.Class).AsQueryable();
        if (classId.HasValue)
            query = query.Where(s => s.ClassId == classId.Value);

        return await query
            .OrderBy(s => s.Name)
            .Select(s => new SubjectDto(s.Id, s.Name, s.Code, s.ClassId, s.Class.Name))
            .ToListAsync(ct);
    }

    public async Task<SubjectDto> GetAsync(Guid id, CancellationToken ct)
    {
        var subject = await _db.Subjects.Include(s => s.Class).FirstOrDefaultAsync(s => s.Id == id, ct)
    ...
    public async Task<SubjectDto> CreateAsync(CreateSubjectRequest request, CancellationToken ct)
    {
        ...
        var exists = await _db.Subjects.AnyAsync(s => s.ClassId == request.ClassId && s.Code.ToLower() == code, ct);
        if (exists)
            throw new AppException("A subject with this code already exists in this class.", 409, "conflict");
        ...
        return new SubjectDto(subject.Id, subject.Name, subject.Code, subject.ClassId, klass.Name);
    }
    ...
    public async Task<SubjectDto> UpdateAsync(Guid id, UpdateSubjectRequest request, CancellationToken ct)
    {
        ...
        var codeTaken = await _db.Subjects
            .AnyAsync(s => s.Id != id && s.ClassId == request.ClassId && s.Code.ToLower() == code, ct);
        if (codeTaken)
            throw new AppException("A subject with this code already exists in this class.", 409, "conflict");
    ...
    private static SubjectDto ToDto(Subject s) => new(s.Id, s.Name, s.Code, s.ClassId, s.Class.Name);
}
